...
nsamples <- 20
set.seed(2021)
s2 <- replicate(nsamples, sample_n(diamonds, N)$price)
s2 <- tibble(sample=seq_len(nsamples),
            mean=colMeans(s2),
            se=apply(s2, 2, sd) / sqrt(N),
            z=(mean - h0mean) / se,
            PValue=2 * (1 - pnorm(abs(z)))) # 2-sided test

filter(s2, PValue < 0.05)
```
